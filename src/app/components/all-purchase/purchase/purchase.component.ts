...
export class PurchaseComponent implements OnInit, OnDestroy {
  ...
  totalPages = 0;
  totalElements = 0;
  totalPurchaseAmountSum: number | null = null;
  startIndex = 0;
  endIndex = 0;
...
,
  ...
  private calculateTotalPurchaseAmount(rows: Purchase[]): number {
    let sum = 0;
  ...
  loadPurchases(): void {
    this.isLoading = true;
    this.totalPurchaseAmountSum = null;
    const params = {
    ...
    this.purchaseService.searchPurchases(params)
      ...
      .subscribe({
        next: (response) => {
          ...
          this.totalPurchaseAmountSum = this.calculateTotalPurchaseAmount(this.purchases);
  ...
  deletePurchase(id: number): void {
    if (confirm(this.txLabel.swap('Are you sure you want to delete this purchase? This action cannot be undone.'))) {
      this.isLoading = true;
      this.purchaseService.deletePurchase(id)
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: () => {
            this.snackbar.success(this.txLabel.swap('Purchase deleted successfully'));
            this.loadPurchases();
          },
          error: (error) => {
