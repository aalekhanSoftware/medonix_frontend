...
export class SaleReturnListComponent implements OnInit, OnDestroy {
  ...
  totalPages = 0;
  totalElements = 0;
  totalSaleReturnAmountSum: number | null = null;
  startIndex = 0;
  endIndex = 0;
  ...
  private calculateTotalSaleReturnAmount(rows: SaleReturn[]): number {
    let sum = 0;
    for (const row of rows ?? []) {
      const value = row?.totalSaleAmount as unknown;
  ...
  loadSaleReturns(): void {
    this.isLoading = true;
    this.totalSaleReturnAmountSum = null;
    ...
    this.saleService.searchSaleReturn(params)
      ...
      .subscribe({
        next: (response: SaleReturnResponse) => {
          ...
          this.totalSaleReturnAmountSum = this.calculateTotalSaleReturnAmount(this.saleReturns);
